 ...
 * - This example demonstrates how to receive encrypted data.
 * - Both sender and receiver must share the same encryption key. A module with a different key (or
 *   key 0x0000) will receive garbled data or nothing at all.
 * - Note: the key is write-only on the module; reading it back always returns 0x0000.
 * - Pair this example with EncryptionSend configured with the same ENCRYPTION_KEY.
...
// This receiver's address and channel: must match DEST_ADDRESS/DEST_CHANNEL in EncryptionSend
...
void setup() {
  ...
  Serial.println("--------------------------------------");
  Serial.println("LoRa-E22T - EncryptionReceive Example");
  Serial.println("--------------------------------------");
